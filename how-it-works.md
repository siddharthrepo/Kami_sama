...
| `find_boilerplate.py` | Optional — finds lines that repeat across the corpus (see §12) |
...
### We checked for repeated boilerplate, and left the corpus alone

Other people's cleaning pipelines include a step we didn't have: removing lines that repeat
across thousands of documents — site footers, "also read" links, bylines. Ours only removed
boilerplate someone had written a pattern for.

So we built the missing step (`scripts/find_boilerplate.py`) and measured it. Scanning
120,000 Nepali documents found 308 repeated lines across 15,009 occurrences — 1.07% of all
lines. The worst offenders were exactly what you'd expect:

```
1218x  'यो पनि पढ्नुहोस्'   ("read this too")
 831x  'एजेन्सी'            ("agency")
 335x  'रासस'               (the national news agency)
 320x  'भिडियो'             ("video")
 230x  'पुष्प दुलाल'         (a journalist's name — a byline)
```

**But measured in tokens rather than lines, it comes to 0.118%** — about 0.8 million out of
659 million. Those lines are the shortest in the corpus, one to four words each. No document
dropped below the quality threshold because of them.

So the corpus was kept as it was. Re-running everything — clean, split, tokenizer, counting —
to remove one eighth of one percent of a corpus already 32% above target was not worth it.
The feature stays in the code, switched off by default, and the measurement is written down
in `report/phase1.md` §8.

**The lesson:** "we didn't do it" and "we did it, measured it, and it wasn't worth it" look
similar from outside and are completely different. Measuring took 20 minutes.
